...
{
	lcdCommand(_BV(LCD_DDRAM)+(X+_BV(LCD_CGRAM)*Y));
}
...
void lcdInit(void)
{
	...
	lcdCommand(_BV(LCD_DISPLAYMODE) | _BV(LCD_DISPLAYMODE_ON  ) );		
}
...
int lcdPrintf(const char *Format, ...)
{
	...
	int Rc = vsprintf(PrivateLcdStructure.InternalBuffer, Format, Args);
	va_end(Args);
	lcdPuts(PrivateLcdStructure.InternalBuffer);
...
{
	lcdCommand(_BV(LCD_DISPLAYMODE) | _BV(LCD_DISPLAYMODE_ON) | _BV(LCD_DISPLAYMODE_BLINK));
}
